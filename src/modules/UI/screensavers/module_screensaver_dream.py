...
import hashlib
from modules.UI.screensavers.module_screensaver_overlay import TimeOverlay
...
class DreamAnimation:
    ...
    def __init__(self, screen, width, height, show_time=False):
        ...
        # ── Fonts ────────────────────────────────────────────────────────
        try:
            self.font_frag = pygame.font.Font(str(__import__("pathlib").Path(__file__).parent.parent / "pixelmix.ttf"), 20)
            self.font_label = pygame.font.Font(str(__import__("pathlib").Path(__file__).parent.parent / "pixelmix.ttf"), 12)
        except Exception:
