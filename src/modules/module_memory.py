...
class MemoryManager:
    ...
    def get_related_memories(self, query: str, include_context: bool = True) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # Build an id-based index for O(1) lookup instead of O(n) linear scan
            doc_id_to_index = {id(doc['document']): idx for idx, doc in enumerate(memory_list)}
            ...
            for i in range(num_to_process):
                ...
                start_index = doc_id_to_index.get(id(memory))
                if start_index is None:
                    # Fallback: linear scan if object identity doesn't match
