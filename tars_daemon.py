...
class TARSDaemon:
    ...
    def _register_routes(self, app: FastAPI):
        ...
        async def set_emotion(request: EmotionRequest):
            """Set display emotion (for dashboard)."""
            if not self.hardware_controller:
                raise HTTPException(503, "Hardware controller not available")

            try:
                result = self.hardware_controller.set_emotion(request.emotion)
                return result
        ...
        @app.post("/api/eye-state")
        async def set_eye_state(request: EyeStateRequest):
            """Set eye state (for dashboard)."""
            if not self.hardware_controller:
                raise HTTPException(503, "Hardware controller not available")

            try:
                result = self.hardware_controller.set_eye_state(request.state)
                return result
            except ValueError as e:
                raise HTTPException(400, str(e))
            except Exception as e:
